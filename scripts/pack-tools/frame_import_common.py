"""Character-pack frame import — #FF00FF magenta workflow.

See tools/template/cursor-rules/character-pack-import.mdc
"""
...
from resolve_paths import REPO_ROOT, frames_dir

ROOT = REPO_ROOT
FRAMES = frames_dir()
MAGENTA_PLATES = REPO_ROOT / "assets" / "magenta-plates"
...
def load_sheet_on_magenta(sheet_path: Path) -> Image.Image:
    ...
    if sheet_is_cursor_chroma(sheet):
        print("  [1/2] chroma sheet - key only, no black paint")
        return sheet
    ...
    print("  [1/2] black sheet -> #FF00FF plate (interim; regenerate with magenta prompt)")
...
def reference_char_height(frames: Path | None = None) -> int:
    out_dir = frames or FRAMES
...
def compose_frame(
    ...
    *,
    ...
    if target_char_h is None:
        target_char_h = reference_char_height(out_dir)
